...
client.on('ready', async () => {
 ...
 client.user.setActivity(`BOT.JS 25.SATIRDAN DÜZELTEBİLİRSİNİZ!`, { type:'PLAYING' }) //PLAYING= OYNUYOR, WHATCHING= İZLİYOR, LISTENING= DİNLİYOR
...
client.on('guildMemberAdd', async (member) => {
    var kanal = member.guild.channels.get('812598687129534469') //Mesajın atılmasını istediğiniz kanalın id sini girin.
    kanal.send(`**Sevgili** ${member},/ Büyük bir zevkle Hogwarts Cadılık ve Büyücülük Okulu'na kabul edildiğinizi bildirmekteyim. Discord üzerindeki telefon doğrulamanızı yaptıktan sonra <#812598687129534468> kanalında kendinizi aşağıdaki gibi tanıtmanız gerekmektedir. n/ **Örnek:** Tuğba / 22/Sly/Rav/Huff/Gry/Kadın n/ Daha sonrasında sol tarafta bulunan sesli kanallardan herhangi birine bağlanıp sunucu hakkında kısa bir rehberlik almalısınız. n/ n/ **Sevgilerle,**
**Parker Dowjin** n/ **Baş Yönetici** `) ;
})
////////////////
...
.setColor('RED')
.setDescription(`${channel.name} adlı Kanal silindi Silen kişinin yetkilerini Aldım. `)
.setFooter('parker')
sChannel.send(cıks)
...
client.on("guildMemberAdd", member => {  
   ...
   let s = moment.duration(kurulus).format('d')
    var kontrol;
  if (s < 15) kontrol = 'Güvenli ✔'
  if (s > 15) kontrol = 'Güvenli Değil ❌'
    moment.locale("tr");
       kanal.send(`${user}`) 
      const tedoa = new Discord.MessageEmbed()
    .setTitle('ÜST_BAŞLIK') 
      .setDescription(`HOŞGELDİN_MESAJI`)
   //.setImage("İSTERSENİZ GİF")//EĞER GİF KOYMAK İSTERSENİZ BAŞINDAKİ //'I KALDIRIN
   kanal.send(tedoa)   
  });
...
client.on("message", async msg => {
  if (msg.content.toLowerCase() === 'sa') {
    msg.inlineReply('As Hoşgeldin!');
  }
});
client.on("message", async msg => {
  if (msg.content.toLowerCase() === 'merhaba') {
    msg.inlinereply('Merhaba Hoşgeldin!!');
  }
});
...
client.on("message", async msg => {
 const i = await db.fetch(`${msg.guild.id}.kufur`)
    if (i) {
        const kufur = ["oç", "amk", "ananı sikiyim", "ananıskm", "piç", "amk", "amsk", "sikim", "sikiyim", "orospu çocuğu", "piç kurusu", "kahpe", "orospu", "mal", "sik", "yarrak", "am", "amcık", "amık", "yarram", "sikimi ye", "mk", "mq", "aq", "yarrak", "amq",];
...
client.on("guildMemberAdd", member => {
  ...
  moment.locale("tr")
   ...
   var kayıtsız = member.guild.roles.cache.get("ALINACAL_ROL_ID") 
   member.roles.add(rol)
member.user.send('Hesabın 3 günden az bir sürede açıldığı için cezalı kanala atıldın!')
setTimeout(() => {
